  ...
  <table class="table table-bordered">
    ...
    <tbody id="productTableBody">
      <?php if ($products->num_rows > 0): ?>
        ...
        ?>
          <tr data-product-id="<?= $row['PId'] ?>">
            <td><?= $row['PId'] ?></td>
            <td><?= $row['PName'] ?></td>
            <td><?= $row['PPrice'] ?></td>
            <td><img src="<?= $row['PImage'] ?>" alt="Product Image" width="60" /></td>
            <td><?= $row['PStatus'] ?></td>
            <td><?= $row['PAdmin_Id'] ?></td>
            <td><?= $row['PCatergory'] ?></td>
            <td><?= $row['PStock'] ?></td>
            <td>
              <a href="add-product.php?edit=1&PId=<?= $row['PId'] ?>&PName=<?= urlencode($row['PName']) ?>&PPrice=<?= $row['PPrice'] ?>&PStatus=<?= urlencode($row['PStatus']) ?>&category=<?= urlencode($row['PCatergory']) ?>&PStock=<?= $row['PStock'] ?>" class="btn btn-warning btn-sm">✏️ Edit</a>
